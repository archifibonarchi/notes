...
const emptyState: Task[] = [
  ...
  {
    id: uid(),
    title: 'Drag me to Middle or Big',
    done: false,
    group: 'today',
    createdAt: Date.now(),
    updatedAt: Date.now(),
...
export default function App() {
  // Минимальный SW, чтобы ставилось на экран и работало офлайн
  useEffect(() => {
  ...
  const [syncKey, setSyncKey] = useState<string>(
    () => localStorage.getItem(LS_SYNC_KEY) || ''
  );
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LS_SYNC_KEY, syncKey);
    } catch {}
  }, [syncKey]);
  ...
  const addTask = () => {
    ...
    const now = Date.now();
    setTasks((t) => [
      { id: uid(), title, done: false, group: newGroup, createdAt: now, updatedAt: now },
      ...t,
    ]);
    setNewTitle('');
    pushSnapshot('add');
  ...
  const toggleDone = (id: string) => {
    setTasks((t) =>
      t.map((x) => (x.id === id ? { ...x, done: !x.done, updatedAt: Date.now() } : x))
    );
    pushSnapshot('toggle');
  };
  ...
  const renameTask = (id: string, title: string) => {
    setTasks((t) =>
      t.map((x) => (x.id === id ? { ...x, title, updatedAt: Date.now() } : x))
    );
    pushSnapshot('rename');
  };
  const moveTaskTo = (id: string, group: Group) => {
    setTasks((t) =>
      t.map((x) => (x.id === id ? { ...x, group, updatedAt: Date.now() } : x))
    );
    pushSnapshot('move');
  };
  ...
  const onDropGroup = (e: React.DragEvent, g: Group) => {
    e.preventDefault();
    const id = dragId.current || e.dataTransfer.getData('text/plain');
    if (id) moveTaskTo(id, g);
    dragId.current = null;
  };
  ...
  function mergeTasks(local: Task[], remote: Task[]): Task[] {
    ...
    [...local, ...remote].forEach((t) => {
      ...
      if (!p || t.updatedAt > p.updatedAt) byId.set(t.id, t);
    });
    ...
    pushDebounce.current = window.setTimeout(async () => {
      try {
        ...
        const { error } = await sb
          .from('boards')
          .upsert({ key: syncKey, data: payload })
          .select();
        if (error) throw error;
        setSyncStatus('idle');
      } catch (e) {
    ...
    <div className="min-h-screen w-full" style={{ background: '#fafafa', color: '#111' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#666' }}>
                Big • Middle • Today — drag to triage, check to complete
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button
              onClick={() => {
                ...
                a.href = url;
                a.download = `tri-notes-${Date.now()}.json`;
                a.click();
                URL.revokeObjectURL(url);
              }}
              style={{ border: '1px solid #ddd', borderRadius: 12, padding: '6px 10px', background: '#fff' }}
            >
              Export
            </button>
            <label
            ...
            <button
              onClick={() => {
                const url = `${location.origin}${location.pathname}#key=${encodeURIComponent(
                  syncKey || 'archi'
                )}`;
            ...
            >
              ...
              <button
                ...
                style={{ background: '#111', color: '#fff', borderRadius: 8, padding: '4px 8px', fontSize: 12 }}
              >
                Pull
              </button>
              <span
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: 16, display: 'flex', gap: 8 }}>
        <input
          ...
          style={{ flex: 1, border: '1px solid #ddd', borderRadius: 12, padding: '8px 12px' }}
        />
        <select
          value={newGroup}
          onChange={(e) => setNewGroup(e.target.value as Group)}
          style={{ border: '1px solid #ddd', borderRadius: 12, padding: '8px 12px' }}
        >
        ...
        <button
          onClick={addTask}
          style={{ background: '#111', color: '#fff', borderRadius: 12, padding: '8px 16px' }}
        >
          Add
        </button>
      </div>
        ...
        {(['big', 'middle', 'today'] as Group[]).map((g) => (
          ...
          >
            <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <h2 style={{ textTransform: 'uppercase', fontSize: 12, letterSpacing: 0.6, color: '#666', fontWeight: 600 }}>
                {groupLabel[g]}
              </h2>
            ...
            <div>
              {groups[g].length === 0 && (
                ...
                >
                  Drop here to move tasks into {groupLabel[g]}
                </div>
              )}
              {groups[g].map((t) => (
                <TaskCard
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <button
          ...
          style={{ fontSize: 12, padding: '4px 8px', borderRadius: 8, border: '1px solid #eee' }}
        >
          Edit
        </button>
        <button
        ...
        <span title="Drag" style={{ fontSize: 12, color: '#999', userSelect: 'none' }}>
          ⇅
        </span>
      </div>
    </div>
